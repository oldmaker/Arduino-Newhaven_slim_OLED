 ...
 *
 * This sketch displays on OLED the text received from the serial port (e.g. from the Serial Monitor)
 *
 ...
 * Modified 17 Jan 2015 by Pasquale D'Antini
...
void loop(void)
{
   ...
   {
      delay(100);                               // waits a bit for the entire message to arrive
      oled.clear();                             // clears the screen
      while (Serial.available() > 0)            // reads all the available characters
      {
         oled.write(Serial.read());             // displays each character on the OLED
      }
   }
}
